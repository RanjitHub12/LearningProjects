...
# --- LIFESPAN MANAGER (The Correct Way to Start DB) ---
@asynccontextmanager
async def lifespan(app: FastAPI):
    # This creates tables ONLY when the server is fully ready
    models.Base.metadata.create_all(bind=database.engine)
    yield
    ...
    return [
        {
            "wpm": r.wpm,
            "accuracy": r.accuracy,
            "date": r.created_at.strftime("%H:%M")
        } 
        for r in history
    ]
...
async def upload_file(file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    ...
    for word_text in words_in_file:
        if len(word_text) > 1 and word_text in spell and not profanity.contains_profanity(word_text):
            try:
                with db.begin_nested():
                    new_word = models.Word(text=word_text)
                    db.add(new_word)
